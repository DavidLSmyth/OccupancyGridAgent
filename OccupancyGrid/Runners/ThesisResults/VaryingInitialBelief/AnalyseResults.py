...
initial_beliefs = ['0.25', '0.75']
for search_strategy in search_strategies:
    for initial_belief in initial_beliefs:
        try:
            os.chdir("D:/OccupancyGrid/OccupancyGrid/Runners/ThesisResults/VaryingInitialBelief/{}/{}".format(initial_belief[2:],search_strategy))
            csv_locs = ["VaryingInitialBeliefProcess{}.csv".format(i) for i in range(1, 9)]    
            data_frames = [pd.read_csv(csv_loc, sep='\t') for csv_loc in csv_locs]
            all_data = pd.concat(data_frames)
            assert len(all_data) == 5000, str(len(all_data)) + ' ' + initial_belief + ' ' +search_strategy
            ...
            plt.savefig("SingleAgentSingleSource{}{}Histogram.png".format(initial_belief[2:], search_strategy))
        except Exception as e:
            print(f"Skipped generation of analysis for {search_strategy} {initial_belief}")
