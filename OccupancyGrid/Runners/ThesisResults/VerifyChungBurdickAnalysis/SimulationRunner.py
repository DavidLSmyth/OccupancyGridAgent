...
#Run the search with the parameters specified in the simulation section of "Analysis of Search Decision Making Using Probabilistic Search Strategies"
...
from Utils.SearchTermination import UpperLowerBoundTotalBeliefSearchTermination


#%%


def main(run_no, prior, search_strategy, initial_belief_present, sensor_model_fpr, sensor_model_fnr):
...
#    agent3_name = 'agent3'
    ...
    search_terminator = UpperLowerBoundTotalBeliefSearchTermination(0.05,0.9)
    ...
    #%%
    results_file = "D:/OccupancyGrid/OccupancyGrid/Runners/ThesisResults/VerifyChungBurdickAnalysis/{}/{}/VerifyChungBurdickAnalysisProcess{}.csv".format(str(sensor_model_fpr)[2:]+'-'+str(sensor_model_fnr)[2:], search_strategy, run_no)
    covariance_matrix = [[3.0, 0], [0, 3.0]]
...
#designed to run in parallel
    ...
    t1 = time.time()
    main(int(run_number), prior, search_strategy, float(initial_belief_present), float(sensor_model_fpr), float(sensor_model_fnr))
    t2 = time.time()
