...
def main(run_no, prior, search_strategy, initial_belief_present, sensor_model_fpr, sensor_model_fnr, no_sources):
...
#    agent3_name = 'agent3'
    ...
    agent1 = MultipleSourceDetectingGridAgent(grid, agent1_start_pos, ss.get_move, -10, agent1_name, agent1_simulated_sensor, MultipleSourceBinaryBeliefMap, agent1_initial_belief_map, search_terminator, other_active_agents = [], comms_radius = 2, logged=False)
    #%%
    ...
    with open(results_file, 'w') as csv_file:
        ...
        for run_number in range(no_runs):
            ...
            #generate new sources positions
            ...
            write_str+=str(no_timesteps) + '\t' + ','.join(list(map(lambda located_source: str(located_source.grid_loc),located_sources))) + '\t' + ','.join(list(map(lambda new_source_position: str(new_source_position), new_source_positions))) + '\n'
...
#designed to run in parallel
    ...
    t1 = time.time()
    main(int(run_number), prior, search_strategy, float(initial_belief_present), float(sensor_model_fpr), float(sensor_model_fnr), int(no_sources))
    t2 = time.time()
