...
def main(run_no, prior, search_strategy):
...
#    agent3_name = 'agent3'
    ...
    initial_belief = generate_gaussian_prior(grid, [source_locations[0].x_val, source_locations[0].y_val], covariance_matrix, initial_belief_sum = 0.5) if prior == 'gaussian' else generate_uniform_prior(grid)
    ...
    #%%
    results_file = "D:/OccupancyGrid/OccupancyGrid/Runners/ThesisResults/Example1/GaussianPrior/Random/Experiment1Process{}.csv".format(run_no)
    covariance_matrix = [[3.0, 0], [0, 3.0]]
    with open(results_file, 'w') as csv_file:
        ...
        for run_number in range(no_runs):
            ...
            agent1.current_belief_map.current_belief_vector.estimated_state = generate_gaussian_prior(grid, [new_source_pos_x, new_source_pos_y], covariance_matrix, initial_belief_sum = 0.5) if prior == 'gaussian' else uniform_initial
...
#designed to run in parallel
    ...
    t1 = time.time()
    main(int(run_number), prior, search_strategy)
    t2 = time.time()
