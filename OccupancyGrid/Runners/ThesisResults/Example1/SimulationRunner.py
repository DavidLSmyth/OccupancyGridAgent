...
#    agent3_name = 'agent3'
    ...
    if prior == 'Gaussian':
        initial_belief = generate_gaussian_prior(grid, [source_locations[0].x_val, source_locations[0].y_val], covariance_matrix, initial_belief_sum = 0.5)
    elif prior == 'Uniform':
        initial_belief = generate_uniform_prior(grid)
    else:
    ...
    #%%
    results_file = "D:/OccupancyGrid/OccupancyGrid/Runners/ThesisResults/Example1/{}/{}/Experiment1Process{}.csv".format(prior, search_strategy, run_no)
    covariance_matrix = [[3.0, 0], [0, 3.0]]
    with open(results_file, 'w') as csv_file:
        ...
        for run_number in range(no_runs):
            ...
            if prior == 'Gaussian':
                agent1.current_belief_map.current_belief_vector.estimated_state = generate_gaussian_prior(grid, [new_source_pos_x, new_source_pos_y], covariance_matrix, initial_belief_sum = 0.5)
            elif prior == 'Uniform': 
