...
def main(run_no, prior, search_strategy, initial_belief_present, sensor_model_fpr, sensor_model_fnr, no_sources, no_agents):
    ...
    agent4_start_pos = get_agent_start_pos_from_config(1)
    agent5_start_pos = get_agent_start_pos_from_config(1)
    ...
    if len(grid.get_grid_points()) < 200:
        ...
        sweep_action_selection_method = TSPActionSelection(grid, agent4_start_pos)
    ...
    agent4_simulated_sensor = get_simulated_sensor_from_config(1)
    agent4_simulated_sensor.source_locations = source_locations
    
    agent5_simulated_sensor = get_simulated_sensor_from_config(1)
    agent5_simulated_sensor.source_locations = source_locations
    
    agent6_simulated_sensor = get_simulated_sensor_from_config(1)
    agent6_simulated_sensor.source_locations = source_locations

    #agent4_sensor_model_fpr, agent4_sensor_model_fnr = get_sensor_model_params_from_config(1).false_positive_rate, get_sensor_model_params_from_config(1).false_negative_rate
    agent4_sensor_model_fpr, agent4_sensor_model_fnr = sensor_model_fpr, sensor_model_fnr
    ...
    #estimated_state_map = create_multiple_source_binary_belief_map(grid, uniform_initial, agent4_sensor_model_fpr, agent4_sensor_model_fnr)
    
    
    agent4_initial_belief_map = MultipleSourceBinaryBeliefMap(grid, initial_belief, agent4_sensor_model_fpr, agent4_sensor_model_fnr)
    agent5_initial_belief_map = MultipleSourceBinaryBeliefMap(grid, initial_belief, agent4_sensor_model_fpr, agent4_sensor_model_fnr)
    agent6_initial_belief_map = MultipleSourceBinaryBeliefMap(grid, initial_belief, agent4_sensor_model_fpr, agent4_sensor_model_fnr)
    ...
    #agent4_initial_belief_map.get_probability_source_in_grid()
    #agent4_initial_belief_map.current_belief_vector.get_estimated_state()
    search_terminator1 = SequentialProbRatioTest(agent4_initial_belief_map.get_probability_source_in_grid(), *get_SPRT_params_from_config(1))
    search_terminator2 = SequentialProbRatioTest(agent5_initial_belief_map.get_probability_source_in_grid(), *get_SPRT_params_from_config(1))
    ...
    if no_agents == 2:
        agent4 = SimpleGridAgent(grid, agent4_start_pos, ss.get_move, -10, agent4_name, agent4_simulated_sensor, MultipleSourceBinaryBeliefMap, agent4_initial_belief_map, search_terminator1, other_active_agents = ["agent5"], comms_radius = 1000, logged=False)
        agent5 = SimpleGridAgent(grid, agent5_start_pos, ss.get_move, -10, agent5_name, agent5_simulated_sensor, MultipleSourceBinaryBeliefMap, agent5_initial_belief_map, search_terminator2, other_active_agents = ["agent4"], comms_radius = 1000, logged=False)
        print("Running Simulation with 2 agents \n ******************************************\n\n\n\n\n")
        occupancy_grid_agents = [agent4, agent5]
        
    elif no_agents == 3:
        agent4 = SimpleGridAgent(grid, agent4_start_pos, ss.get_move, -10, agent4_name, agent4_simulated_sensor, MultipleSourceBinaryBeliefMap, agent4_initial_belief_map, search_terminator1, other_active_agents = ["agent5", "agent6"], comms_radius = 1000, logged=False)
        agent5 = SimpleGridAgent(grid, agent5_start_pos, ss.get_move, -10, agent5_name, agent5_simulated_sensor, MultipleSourceBinaryBeliefMap, agent5_initial_belief_map, search_terminator2, other_active_agents = ["agent4", "agent6"], comms_radius = 1000, logged=False)
        agent6 = SimpleGridAgent(grid, agent6_start_pos, ss.get_move, -10, agent6_name, agent6_simulated_sensor, MultipleSourceBinaryBeliefMap, agent6_initial_belief_map, search_terminator3, other_active_agents = ["agent4", "agent5"], comms_radius = 1000, logged=False)
        print("Running Simulation with 3 agents \n ******************************************\n\n\n\n\n")
        occupancy_grid_agents = [agent4, agent5, agent6]
    #%%
    ...
    with open(results_file1, 'w') as csv_file1, open(results_file2, 'w') as csv_file2, open(results_file3, 'w') as csv_file3:
        ...
        for run_number in range(no_runs):
            #agent5 = MultipleSourceDetectingGridAgent(grid, agent4_start_pos, epsilon_greedy_action_selection_method.get_move, -10, agent4_name, agent4_simulated_sensor, MultipleSourceBinaryBeliefMap, agent4_initial_belief_map, search_terminator, other_active_agents = [], comms_radius = 2, logged=False)
            #t1 = time.time()
            write_str1 = ''
            write_str2 = ''
            write_str3 = ''
            #new_agent4_start_pos = Vector3r(random.randint(0, 9) , random.randint(0, 9))
            #new_agent5_start_pos = Vector3r(random.randint(0, 9) , random.randint(0, 9))
            ...
            #agent4.current_pos_intended = new_agent4_start_pos
            #agent5.current_pos_intended = new_agent5_start_pos
            #agent6.current_pos_intended = new_agent6_start_pos
            
            
            agent4_simulated_sensor.source_locations = list(new_source_positions)
            agent5_simulated_sensor.source_locations = list(new_source_positions)
            agent6_simulated_sensor.source_locations = list(new_source_positions)

            #print(agent4_simulated_sensor.source_locations)
            #agent4.current_belief_map = MultipleSourceBinaryBeliefMap(grid, uniform_initial, agent4_sensor_model_fpr, agent4_sensor_model_fnr)
            if prior == 'Gaussian':
                for agent in occupancy_grid_agents:
            ...
            #agent4.occupancy_sensor_simulator = agent4_simulated_sensor
            #print(agent4.current_belief_map.current_belief_vector.get_estimated_state())

            #print("source_location: ", new_source_positions)
            #located_sources = agent4.find_sources(no_sources)
            ...
            #located_sources_agent4 = agent4.current_belief_map.get_ith_most_likely_component(1)
            #located_sources_agent5 = agent5.current_belief_map.get_ith_most_likely_component(1)
            ...
            write_str1+=str(agent4.timestep) + '\t' + str(located_sources[0].grid_loc) + '\t' + str(new_source_location) + '\n'
            write_str2+=str(agent5.timestep) + '\t' + str(located_sources[1].grid_loc) + '\t' + str(new_source_location)  + '\n'
