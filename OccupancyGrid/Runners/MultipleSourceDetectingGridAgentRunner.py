...
if __name__ == '__main__':
    ...
    #grid: UE4Grid, prior: np.array, sensor_model_fpr: 'prob of false pos', sensor_model_fnr: 'prob of false neg'
    agent1_intial_estimated_state = MultipleSourceBinaryBeliefMap(grid, uniform_initial, agent1_sensor_model_fpr, agent1_sensor_model_fnr)
    #grid, initial_pos, move_from_bel_map_callable, height, agent_name, occupancy_sensor_simulator, belief_map_class, init_belief_map, search_terminator, other_active_agents = [], initial_estimated_state = np.array([]), comms_radius = 1000, logged = True
    agent1 = MultipleSourceDetectingGridAgent(grid, agent1_start_pos, sweep_action_selection_method.get_move, -10, agent1_name, agent1_simulated_sensor, MultipleSourceBinaryBeliefMap, agent1_intial_estimated_state, other_active_agents = [], comms_radius = 2, prior = uniform_initial, logged=False, single_source=single_source)
