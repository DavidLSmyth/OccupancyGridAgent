...
class VehicleClient:
    ...
    def get_vehicles_in_range(self, vehicle_name, otherVehiclesNames, range_m):
    ...
    def can_coord_with_other(self, vehicle_name, otherVehicleName, range_m):
        return otherVehicleName in self.get_vehicles_in_range(vehicle_name, [otherVehicleName], range_m)
    
    def get_vehicles_in_range_stochastic(self, vehicle_name, other_vehicles_names, range_m):
        get_vehicle_distance = lambda other_vehicle_name: (self.simGetGroundTruthKinematics(other_vehicle_name).position).distance_to(self.simGetGroundTruthKinematics(vehicle_name).position)
        return list(filter(lambda other_vehicle_name: get_vehicle_distance(other_vehicle_name)/(range_m) < random.random() if get_vehicle_distance(other_vehicle_name) < range_m else False, other_vehicles_names))
        #return list(filter(lambda other_vehicle_name: (self.simGetGroundTruthKinematics(other_vehicle_name).position).distance_to((self.simGetGroundTruthKinematics(vehicle_name).position)) < range_m, other_vehicles_names))

    def can_coord_with_other_stochastic(self, vehicle_name, otherVehicleName, range_m):
        return otherVehicleName in self.get_vehicles_in_range_stochastic(vehicle_name, [otherVehicleName], range_m)
    # -----------------------------------  Common vehicle APIs ---------------------------------------------
    def reset(self):
        self.battery = BatteryState()
